...
} from '../generated/Arts_FoundationFactory/NFTCollectionFactory';
...
export function handleTransfer(event: Transfer): void {
  ...
  if (contract === null) {
    ...
    // Blockchain
    blockchain.totalContracts = blockchain.totalContracts.plus(
      BigInt.fromI32(1)
    );
    blockchain.save();
  }
  ...
  if (contract.name === 'unknown') return;

  blockchain.totalTransactions = blockchain.totalTransactions.plus(
    BigInt.fromI32(1)
  );
  blockchain.save();
  contract.totalTransactions = contract.totalTransactions.plus(
    BigInt.fromI32(1)
  );
  contract.updatedAt = event.block.timestamp;
  ...
  if (token === null) {
    ...
    if (isKnown == null) {
      token.minter = event.params.from.equals(Address.zero()) ? to.id : from.id;
    } else {
...
export function handleDoidTransfer(event: Transfer): void {
  if (event.params.from.equals(Address.zero())) {
    // register case
    return;
  }
...
export function handlerFoundationNFTDropCollectionCreated(
  event: NFTDropCollectionCreated
): void {
  ERC721.bind(event.params.collection);
}

export function handlerFoundationNFTCollectionCreated(
